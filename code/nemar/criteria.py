...
class ConsistencyLoss(nn.Module):
    def __init__(self, device, img_shape, threshold=70 / 127, vgg_layer_idx=21, c_weight=1e-7):
        ...
        self.i_loss = InvertibilityLoss()#mse
        self.g_loss = GrayscaleConformityLoss(device, img_shape, threshold, vgg_layer_idx, c_weight)
        self.q_loss = QuantizationLoss()

    def forward(self, gray_img, ref_img, loss_stage, s_weight):

        if loss_stage == 1:
            # i_rightloss=self.i_loss(gray_img, ref_img)
            g_loss = self.g_loss(gray_img, ref_img, ls_weight=s_weight)
     
            total_loss =  g_loss  # 3 channels
        elif loss_stage == 2:
            g_loss = self.g_loss(gray_img, original_img, ls_weight=s_weight)
            q_loss = self.q_loss(gray_img)
            total_loss =  g_loss + (10 * q_loss)
        else:
            total_loss = 0
...
#         self.dis = nn.MSELoss(reduction="sum")
        self.vgg = vgg19(pretrained=True).features[:vgg_layer_idx].to(device)
    ...
    def contrast(self, gray_img, original_img):
        ...
        vgg_o = self.vgg(_rescale(original_img.repeat(1, 3, 1, 1)))
    ...
    def forward(self, gray_img, ref_img, ls_weight):
        ...
        c_loss = self.contrast(gray_img, original_luminance)
        ...
        return 10000*l_loss + 0.0001*(self.c_weight * c_loss) + 100*(ls_weight * ls_loss)
