...
from nemar.criteria import ConsistencyLoss
import nemar.stn as stn
import nemar.pwc as pwc
...
class NEMARModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        if self.isTrain:
            ...
            self.loss_cons = ConsistencyLoss(device=str(self.device), img_shape=(opt.batch_size, 1, opt.crop_size, opt.crop_size), c_weight=1)
            self.criterionGAN = networks.GANLoss(opt.gan_mode).to(self.device)  # define GAN loss.
            self.criterionL1 = torch.nn.L1Loss()
            self.criterionL2 = torch.nn.MSELoss()
            self.restore_loss = nn.MSELoss(reduction="sum")
    ...
    def define_networks(self):
        ...
        # print(opt.gpu_ids)
        # time.sleep(1000)
        self.netR = pwc.H_estimator(opt.batch_size,self.opt.crop_size,opt.gpu_ids,self.isTrain)
        self.netR = self.netR.cuda(device=opt.gpu_ids)
        ...
        # print(opt.multi_resolution)
        # print(len(self.netD_multiresolution))
        # time.sleep(1000)
    ...
    def set_input(self, vis,ir,gt):
        ...
        if AtoB:
            self.real_A = vis.to(self.device)
            self.real_B = ir.to(self.device)
            # self.image_paths = input['A_paths']
        else:
            self.real_A = ir.to(self.device)
            self.real_B = vis.to(self.device)
            # self.image_paths = input['B_paths']
    ...
    def forward(self):
        ...
        '''
        tensor_c=self.real_A
        tensor_g=self.real_B
        tensor_prg=self.fake_B

        '''
        tensor_x, self.middle_f = self.netT(x=[self.real_A], rev=False)
        self.fake_B = tensor_x[:, [0], :, :]
        tensor_z = tensor_x[:, 1:, :, :]

        self.tensor_z_ = torch.randn_like(tensor_z).to(tensor_z.device)
        # tensor_g_ = quantize(self.fake_B)

        tensor_x_ = torch.cat((self.fake_B, self.tensor_z_), dim=1)
        self.tensor_y,_ = self.netT(x=[tensor_x_], rev=True)

        tensor_realir_ = torch.cat((self.real_B, self.tensor_z_), dim=1)
        self.tensor_fake_A, self.middle_b = self.netT(x=[tensor_realir_], rev=True)
        self.tensor_fake_A = quantize(self.tensor_fake_A)

        # tensor_xtrue_ = torch.cat((self.real_B, self.tensor_z_), dim=1)
        # tensor_ytrue = self.netT(x=[tensor_xtrue_], rev=True)[0]
        # self.tensor_ytrue_ = quantize(tensor_ytrue)
        
        # self.fake_B = self.netT(self.real_A)
        # wraped_images, reg_term = self.netR(self.real_A, self.real_B.repeat(1, 3, 1, 1),self.opt.crop_size, self.gt, apply_on=[self.real_A, self.fake_B])
        # self.offset1, self.offset2, self.offset3, warp1,warp2,self.fake_RT_B = self.netR(self.real_B,self.real_A,self.fake_B,self.gt)#用假红外input2和真红外进行估计仿射参数并变形得到warped_image
        self.offset1, self.offset2, self.offset3, self.warp1,self.warp2, self.fake_RT_B= self.netR(self.middle_f, self.middle_b,\
                                                                                                   self.real_B.repeat(1, 3, 1, 1),self.real_A,self.fake_B,self.gt)#用假红外input2和真红外进行估计仿射参数并变形得到warped_image
        # self.offset1, self.offset2, self.offset3, self.warp1,self.warp2, self.fake_RT_B= self.netR(self.real_B.repeat(1, 3, 1, 1),self.real_A,self.fake_B,self.gt)#用假红外input2和真红外进行估计仿射参数并变形得到warped_image
        self.registered_real_A = self.warp2[4]
        self.warp_gt = self.warp2[3]
        # self.stn_reg_term = reg_term
        # self.registered_real_A = wraped_images[0]
        # Registration first -- Then --> Translation

        tensor_x2,_ = self.netT(x=[self.registered_real_A], rev=False)
        self.fake_TR_B = tensor_x2[:, [0], :, :]
        ...
        #         self.deformation_field_A_to_B = self.netR.get_grid(self.real_A, self.real_B)
    ...
    def backward_T_and_R(self):
        ...
        self.loss_invt_RT = self.loss_cons(gray_img=self.fake_RT_B, ref_img=self.real_B, 
                                  loss_stage=1, s_weight=1) / self.opt.batch_size
        self.loss_invt_TR = self.loss_cons(gray_img=self.fake_TR_B, ref_img=self.real_B, 
                                  loss_stage=1, s_weight=1) / self.opt.batch_size
        self.loss_fakeA = self.criterionL1(self.tensor_fake_A, self.warp2[3]) #/ self.opt.batch_size
        # loss_self = (tensor_z**2).sum() / (crop_size**2 * 2 * n)
        # loss_gray = loss_dist(self.fake, tensor_g) / n
        self.loss_restore = self.restore_loss(self.tensor_y,self.real_A)
        # loss_vis = loss_dist(tensor_c, tensor_ytrue_) / n

        self.loss_L1_RT_pwc=32*self.criterionL1(self.warp1[0], self.warp2[0])+\
                            16*self.criterionL1(self.warp1[1], self.warp2[1])+\
                            8 *self.criterionL1(self.warp1[2], self.warp2[2])

        self.loss_L1_TR = self.opt.lambda_recon * self.criterionL1(self.fake_TR_B, self.real_B)
        self.loss_L2_gt = 0.1* self.criterionL2(self.offset3, self.gt)
        # ----> GAN loss:
        fake_AB_t = torch.cat((self.real_A, self.fake_TR_B.repeat(1, 3, 1, 1)), 1)
        ...
        # ----> GAN loss:
        fake_AB_t = torch.cat((self.real_A, self.fake_RT_B.repeat(1, 3, 1, 1)), 1)
        pred_fake = self.netD(fake_AB_t)
        ...
        loss = self.loss_L1_TR + self.loss_L1_RT + self.loss_GAN_TR + self.loss_GAN_RT +self.loss_L1_RT_pwc+self.loss_L2_gt#+ self.loss_smoothness
        loss = loss+ 0.5*(0.01*self.loss_invt_RT+0.01*self.loss_invt_TR+  self.loss_restore) + 10*self.loss_fakeA
        loss.backward()
        torch.nn.utils.clip_grad_norm_(self.netT.parameters(), 10)
        return loss

    def backward_D(self):
        """Calculate GAN loss for the discriminator"""
        # Real
        real_AB = torch.cat((self.real_A, self.real_B.repeat(1, 3, 1, 1)), 1)
        ...
        fake_AB = torch.cat((self.real_A, self.fake_TR_B.repeat(1, 3, 1, 1)), 1)
        ...
        for i in range(self.opt.multi_resolution - 1):
            ...
            real_A_resized = F.interpolate(self.real_A, (sh, sw), mode='bilinear', align_corners=False)
            fake_B_B_resized = F.interpolate(self.fake_TR_B, (sh, sw), mode='bilinear', align_corners=False)
            fake_AB_t = torch.cat((real_A_resized, fake_B_B_resized.repeat(1, 3, 1, 1)), 1)
            pred_fake = self.netD_multiresolution[i](fake_AB_t.detach())
            self.loss_D_fake_TR += self.criterionGAN(pred_fake, False)

        # Translation First (RT):
        # ----> Fake
        fake_AB = torch.cat((self.real_A, self.fake_RT_B.repeat(1, 3, 1, 1)), 1)
        pred_fake = self.netD(fake_AB.detach())
        ...
        for i in range(self.opt.multi_resolution - 1):
            ...
            real_A_resized = F.interpolate(self.real_A, (sh, sw), mode='bilinear', align_corners=False)
            fake_B_P_resized = F.interpolate(self.fake_RT_B, (sh, sw), mode='bilinear', align_corners=False)
            fake_AB_t = torch.cat((real_A_resized, fake_B_P_resized.repeat(1, 3, 1, 1)), 1)
            pred_fake = self.netD_multiresolution[i](fake_AB_t.detach())
            self.loss_D_fake_RT += self.criterionGAN(pred_fake, False)
    ...
    def get_current_losses(self):
        return self.loss_L1_TR , self.loss_L1_RT , self.loss_GAN_TR , self.loss_GAN_RT ,0.005*self.loss_invt_RT, 0.005*self.loss_invt_TR,   0.5*self.loss_restore,self.loss_L1_RT_pwc,self.loss_L2_gt,10*self.loss_fakeA
        # return self.loss_L1_TR , self.loss_L1_RT , self.loss_GAN_TR , self.loss_GAN_RT , self.loss_smoothness,0.005*self.loss_invt_RT, 0.005*self.loss_invt_TR,   0.5*self.loss_restore
        
         
    def get_current_visuals(self):
        # print(type(quantize(self.fake_B)))
        # print(type(self.fake_B))
        # time.sleep(1000)
        # print(type(quantize(self.fake_B).repeat(1, 3, 1, 1)))
        return self.real_A, self.real_B.repeat(1, 3, 1, 1), quantize(self.fake_B).repeat(1, 3, 1, 1), quantize(self.tensor_y),quantize(self.fake_TR_B).repeat(1, 3, 1, 1),quantize(self.fake_RT_B).repeat(1, 3, 1, 1),self.registered_real_A,self.warp_gt,self.warp1,self.warp2,self.tensor_fake_A
    def save_networks(self,i,model_path):
        torch.save(self.netD.state_dict(), os.path.join(model_path, 'net_epoch_' + str(i + 1).zfill(3) + '_D.pth'))
        # torch.save(self.netD_multiresolution.state_dict(), os.path.join(model_path, 'net_epoch_' + str(i + 1).zfill(3) + '_D_multiresolution.pth'))
        torch.save(self.netR.state_dict(), os.path.join(model_path, 'net_epoch_' + str(i + 1).zfill(3) + '_R.pth'))
    ...
    def forward_test(self):
        ...
        '''
        with torch.no_grad():
            tensor_x, self.middle_1_f= self.netT(x=[self.real_A], rev=False)
            self.fake_B = tensor_x[:, [0], :, :]
            tensor_z = tensor_x[:, 1:, :, :]
            ...
            tensor_x_ = torch.cat((self.fake_B, self.tensor_z_), dim=1)
            self.tensor_y,_ = self.netT(x=[tensor_x_], rev=True)
            # self.tensor_y=self.real_A

            tensor_realir_ = torch.cat((self.real_B, self.tensor_z_), dim=1)
            self.tensor_fakey, self.middle_1_b = self.netT(x=[tensor_realir_], rev=True)
 
            self.offset1, self.offset2, self.offset3, self.warp1,self.warp2, self.fake_RT_B= self.netR(self.middle_1_f, self.middle_1_b,\
                                                                                                    self.real_B.repeat(1, 3, 1, 1),self.real_A,self.fake_B)
            self.registered_real_A = self.warp2[-1]
    
        # self.stn_reg_term = reg_term
        # self.registered_real_A = wraped_images[0]
        # Registration first -- Then --> Translation

        # tensor_x2,_,_,_ = self.netT(x=[self.registered_real_A], rev=False)
        # self.fake_TR_B = tensor_x2[:, [0], :, :]
        # print("ok")
        # time.sleep(1000)
        #repeat
